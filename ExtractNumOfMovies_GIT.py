...
def write_page_result_in_csv_file(file_output, *args):
    film_file = csv.writer(file_output, delimiter=',', quotechar='\"', quoting=csv.QUOTE_ALL)
...
if __name__ == '__main__':
    ...
    with open('films.csv', 'w', newline='') as file_output:
        while not last_page:
            page_number += 1
            ...
            if page_number == 1:
                ...
                print('Results: ', n_results_in_current_year)
            ...
            for i in range(len(actors_and_genres)):
                ...
                if k != str(current_year):
                    ...
                    write_page_result_in_csv_file(file_output, all_TITLES_in_page[j], all_DIRECTORS_in_page[j], genere, attori)
                    attori.clear()
                    j +=1
